#!/usr/bin/env python3
"""Validates a results.csv against the acceptance criteria: <=10% error on
max_allocated_gb for every non-OOM row, and a correct OOM call on every row.

Reads predicted_allocated_gb / predicted_oom straight out of results.csv (columns
benchmark.py already writes from predictions.py) -- this script doesn't recompute
anything, it just checks what was written against what was measured. No GPU needed.
"""
import argparse
import csv
import sys
...
def print_report(report_rows):
    header = f"{'config':<28} {'predicted_gb':>12} {'measured_gb':>12} {'abs_err_gb':>11} {'pct_err':>9} {'oom_call':>10}"
    print(header)
    print("-" * len(header))
    for r in report_rows:
        if r["oom"]:
            oom_call = "correct" if r["oom_correct"] else "WRONG"
            print(f"{r['label']:<28} {'OOM':>12} {'OOM':>12} {'--':>11} {'--':>9} {oom_call:>10}")
            continue
        oom_call = "correct" if r["oom_correct"] else "WRONG"
        flag = "" if r["pct_error"] <= ERROR_THRESHOLD else "  <-- FAIL"
        print(f"{r['label']:<28} {r['predicted_gb']:>12.3f} {r['measured_gb']:>12.3f} "
...
def main():
    ...
    n_over_threshold = sum(1 for r in report_rows if not r["oom"] and r["pct_error"] is not None and r["pct_error"] > ERROR_THRESHOLD)
    n_oom_call_wrong_on_non_oom = sum(1 for r in report_rows if not r["oom"] and not r["oom_correct"])

    print()
    print(f"Acceptance: <=10% error on max_allocated_gb (non-OOM rows), correct OOM call on all rows.")
